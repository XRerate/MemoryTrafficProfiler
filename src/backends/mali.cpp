...
class MaliBackend::Impl {
 public:
  Impl() : gpu_(0), is_profiling_(false), last_timestamp_ns_(0) {}

  ~Impl() {
    if (sampler_ && is_profiling_) {
      // Best-effort cleanup in destructor: we can't throw exceptions or return
      // errors The error code is captured but not used because:
      // 1. Destructors should be noexcept (can't throw)
      // 2. Object is being destroyed anyway, so error handling is not
      // meaningful
  ...
  bool initialize() {
    ...
    // - MaliExtBusRdBy = MaliExtBusRdBt * MALI_CONFIG_EXT_BUS_BYTE_SIZE (read
    // bytes, absolute)
    // - MaliExtBusWrBy = MaliExtBusWrBt * MALI_CONFIG_EXT_BUS_BYTE_SIZE (write
    // bytes, absolute) These counters return instantaneous bandwidth in
    // bytes/second, not cumulative bytes We use them directly and convert to
    // MB/s
    std::error_code ec;
    ...
    if (ec) {
      fprintf(stderr,
              "DEBUG: Write counter MaliExtBusWrBy not available, "
  ...
  bool start() {
    ...
    if (ec) {
      // Clean up on error: stop sampling since we started it but failed to take
      // initial sample The error code from stop_sampling() is captured but not
      // used because:
      // 1. We're already returning false to indicate start() failed
      // 2. The stop_sampling() error doesn't change the fact that start()
      // failed
      // 3. This is a cleanup attempt - if it fails, there's nothing more we can
      // do
    ...
    // No need to establish baseline - counters are instantaneous, not
    // cumulative
  ...
  bool sample(BandwidthData& data) {
    ...
    if (read_sample.type == hwcpipe::counter_sample::type::uint64) {
      current_read_bytes_per_sec =
          static_cast<double>(read_sample.value.uint64);
    ...
    if (write_sample.type == hwcpipe::counter_sample::type::uint64) {
      current_write_bytes_per_sec =
          static_cast<double>(write_sample.value.uint64);
    } else if (write_sample.type == hwcpipe::counter_sample::type::float64) {
      current_write_bytes_per_sec = write_sample.value.float64;
    }

    data.read_bandwidth_mbps = (current_read_bytes_per_sec / delta_time_sec) /
                               BandwidthConversion::BYTES_TO_MB;
    data.write_bandwidth_mbps = (current_write_bytes_per_sec / delta_time_sec) /
                                BandwidthConversion::BYTES_TO_MB;
    data.total_bandwidth_mbps =
        data.read_bandwidth_mbps + data.write_bandwidth_mbps;
...
bool MaliBackend::initialize() { return pimpl_->initialize(); }

bool MaliBackend::start() { return pimpl_->start(); }

bool MaliBackend::stop() { return pimpl_->stop(); }

bool MaliBackend::sample(BandwidthData& data) { return pimpl_->sample(data); }

bool MaliBackend::is_profiling() const { return pimpl_->is_profiling(); }
